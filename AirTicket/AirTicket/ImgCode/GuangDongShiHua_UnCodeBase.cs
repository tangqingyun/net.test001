...
{
    class GuangDongShiHua_UnCodeBase
    {
        public Bitmap bmpobj;
        public GuangDongShiHua_UnCodeBase(Bitmap pic)
        {
        ...
        ///  图片放大
        /// </summary>
        /// <param name="iSize">放大倍数</param>
        /// <returns></returns>
        public Bitmap GetPicChangeBigValue(int iSize)
        {
            int w = bmpobj.Width * iSize;
            int h = bmpobj.Height * iSize;
            bmpobj = new Bitmap(bmpobj, w, h);
            return bmpobj;
        }
        ...
        public Bitmap KiCut(Bitmap b, int StartX, int StartY, int iWidth, int iHeight)
        {
            if (b == null)
            {
                return null;
        ...
        {
            //由于得到的图片是个8位的索引图所以需要转换成RGB格式的才可以对其进行灰度处理
            image = new Bitmap(image.Width, image.Height, System.Drawing.Imaging.PixelFormat.Format16bppRgb555);
            //原来图片的长度
...
* @return 去色后的图片
*/
        public Bitmap toGrayscale(Bitmap bmpOriginal, string str)
        {
