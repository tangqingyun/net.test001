...
function checkAysnSuggest() {
    $.ajax(
			  {
			      url: ctx + '/loginAction.do?method=loginAysnSuggest',
			      type: "POST",
			      dataType: "json",
...
function checkName() {
    $("#nameSpan").empty();
    if (!checkempty($("#UserName").val())) {
        $("#nameSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>登录名必须填写!</span>");
        $("#UserName").focus();
        return false;
    } else if (!checkUserName()) {
        $("#nameSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>登录名格式不正确，请重新输入!</span>");
        $("#UserName").focus();
        return false;
    } else {
        $("#nameSpan").empty();
...
function checkPassword() {
    ...
    if (!checkempty($("#password").val())) {
        $("#passwordSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>密码必须填写,且不少于6位!</span>");
        $("#password").focus();
        return false;
    } else if (!checkPassLength()) {
        $("#passwordSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>密码长度不能少于6位!</span>");
        $("#password").focus();
        return false;
    } else {
...
function checkRandCode() {
    ...
    if (!checkempty($("#randCode").val())) {
        ...
        $("#randCodeSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>验证码不能为空!</span>");
        $("#randCode").focus();
        return false;
    } else if ($("#randCode").val().length != 4) {
        $("#randCodeSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>验证码长度为4位!</span>");
        $("#randCode").focus();
        return false;
    } else if (!checkNumOrChar($("#randCode").val())) {
        $("#randCodeSpan").html("<img src='images/er/tools_ico2.gif' width='16' height='16' /><span>验证码只能由数字或字母组成!</span>");
        $("#randCode").focus();
        return false;
    } else {
        $("#randCodeSpan").empty();
...
function refreshImg() {
    $("#img_rrand_code").attr("src", "passCodeNewAction.do?module=login&rand=sjrand" + '&' + Math.random());
