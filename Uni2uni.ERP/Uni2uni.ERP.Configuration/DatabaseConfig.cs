...
namespace Uni2uni.ERP.Configuration
{
    public static class DatabaseConfig
    {
        ...
        private static readonly DatabaseSectionHandler _databaseSectionHandler;

        static DatabaseConfig()
        {
            string databaseConfig = GlobleConfig.ConfigFilesBasePath + @"\Database.config";
            var fileMap = new ExeConfigurationFileMap { ExeConfigFilename = databaseConfig};
            var configuration = ConfigurationManager.OpenMappedExeConfiguration(fileMap, ConfigurationUserLevel.None);
            try
            {
                _databaseSectionHandler = (DatabaseSectionHandler)configuration.GetSection("database");
